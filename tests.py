# -------------------------CASOS QUE DAN PROBLEMAS-----------------------------------------
tests = []
test1 = '''type Range(min:Number, max:Number) {
                min = min;
                max = max;
                current = min - 1;
...
tests.append(test2)
test3 = '''type PolarPoint(phi, rho) inherits Point(rho * sin(phi), rho * cos(phi)) {
...
tests.append(test3)
test4 = '''type Knight inherits Person {
                name() => "Sir" @@ base();
            }
            {
                let a = 42 in if (a % 2 == 0) print("Even") else print("odd");
...
tests.append(test4)
test5 = '''type Person(firstname, lastname) {
                firstname = firstname;
                lastname = lastname;
...
# ------------------------- PRUEBAS -----------------------------------------
testX = '''function tan(x: Number): Number => sin(x) / cos(x);
...
customTest = '''function tan(x: Number): Number => sin(x) / cos(x);
                function cot(x) => 1 / tan(x);
                function operate(x, y) {
                    print(x + y);
                    print(x - y);
